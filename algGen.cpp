...
//comandos de entrada e saída de dados na tela
#include <fstream>
		...
		case 0 : //Reiniciar
		{
			...
			while( numIndiv < tamPop) {
			 	for (gene = 0; gene < lcrom; gene++) {
		     		popVelha.indiv[numIndiv].cromossomo[gene] = rand() % 2 ; 							// inteiro aleatorio entre 0 e 1
				}
...
int main(void) {
	...
	for (num_ind=0;num_ind<tamPop;num_ind++){
		popVelha.indiv[num_ind].cromossomo = aloc_vectori(lcrom);
		popNova.indiv[num_ind].cromossomo = aloc_vectori(lcrom);
	}
	arq_melhor_individuo = aloc_matrixi(maxGen+1,lcrom);
	...
	desaloc_matrixi(arq_melhor_individuo,maxGen+1);
