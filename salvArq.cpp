...
void arq_saida(int nroExec)
{
   ...
   // Fitness do Melhor Individuo de Cada Geracao
	...
	sprintf(nome,"MelhoresIndividuos_%d.dat",nroExec); 
	arq = fopen(nome, "at+");
	...
	if(arq != NULL)
	{
		for(i = 1; i <= maxGen ; i++) {
			for(gene = 0; gene < lcrom; gene++)
				fprintf(arq, "%d ",arq_melhor_individuo[i][gene] ); 
			fprintf(arq, "\n");
		}
...
void salv_pop(int nroExec)
{
	...
	sprintf(nome,"pop_%d.dat",nroExec); 
	...
	if(Pop_arq != NULL)
	{
		...
		for(i = 0; i < tamPop; i++){
			for(gene = 0; gene < lcrom ; gene++)
				fprintf(Pop_arq, "%d ",popVelha.indiv[i].cromossomo[gene]);
...
void ler_pop(int nroExec)
{
	...
	while( numIndiv < tamPop) {
	 	for (gene = 0; gene < lcrom; gene++) {
     		fscanf(Pop_arq,"%d", &popVelha.indiv[numIndiv].cromossomo[gene]); 		// inteiro entre 0 e 1
		}
